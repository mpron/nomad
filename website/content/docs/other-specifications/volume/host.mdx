...
- `name` `(string: <required>)` - The name of the volume, which will be used as
  ...
  this volume. Host volume names must be unique per node.
...
- `node_pool` `(string: <optional>)` - A specific node pool where you would like
  ...
  section. Optional for volume creation or volume registration. If you provide
  also `node_id`, the node must be in the provided `node_pool`.
...
[`host_path`](#host_path) fields. The [`constraint`](#constraint) field is
ignored. The [`node_pool`](#node_pool), [`capacity_max`](#capacity_max), and
[`capacity_min`](#capacity_min) fields are ignored but must be consistent if
set; the `node_pool` must match the node set by `node_id`, and the minimum
capacity must be less than the maximum capacity and actual capacity.

## Volume Placement

The `volume create` command will create the volume on a specific node. If
...
* The node must meet the constraints defined by the `constraint` fields.
...
```shell-session
$ nomad volume status 0c903229 | grep Capacity
Capacity             = 50 GiB
```
